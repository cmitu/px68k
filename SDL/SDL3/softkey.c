...
void Soft_kbd_CreateScreen(void)
{
	...
	sft_kbd_render  = SDL_CreateRenderer(sft_kbd_window, NULL );
	...
	keydraw_buffer = SDL_CreateSurface( 800, 220,SDL_PIXELFORMAT_RGBX8888);//描画用
...
{
	SDL_DestroySurface(keydraw_buffer);
...
void draw_soft_kbd(uint32_t ms_x,uint32_t ms_y, uint8_t keyboardLED)
{
	...
	// キーボードの背景(全画面塗りつぶし)
	SDL_FillSurfaceRect(keydraw_buffer, NULL, skb_backcolor);
	...
	for (i = 0; kbd_key[i].x != -1; i++) {
	  ...
	  if((ms_x == 0)&&(ms_y == 0)){// key更新なし(LEDのみ)
		...
		SDL_FillSurfaceRect(keydraw_buffer, &keyrect, 0x00000000);//shadow
	...
	for(i=0; i<7; i++){
	 if(!(y & 0x01)){//かな
	   ...
	   SDL_FillSurfaceRect(keydraw_buffer, &keyrect, (0x00000000 | 0x00ff0000 | 0x00000000));//緑
	 }
	 y >>= 1;
	}
