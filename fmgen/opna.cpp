...
{
	static const int8_t table[3][2] = { { 6, 4 }, { 3, 2 }, { 2, 1 } };
...
void OPNABase::MakeTable2()
{
	if (!tablehasmade)
	{
		for (int_fast16_t i=-FM_TLPOS; i<FM_TLENTS; i++)
		{
...
void OPNB::InitADPCMATable()
{
	...
	for (int_fast16_t i=0; i<=48; i++)
	{
		int32_t s = int32_t(16.0 * pow (1.1, i) * 3);
		for (int_fast16_t j=0; j<16; j++)
		{
